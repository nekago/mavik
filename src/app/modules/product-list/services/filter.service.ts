...
export class FilterService {
  ...
  public modifyFilterState(filters: Filters) {
    ...
    const activeteAllValuesInGroup = (values: FilterFieldsGroupValue[]): FilterFieldsGroupValue[] =>
      values.map(value => ({
        ...value,
        isActive: true,
      }));
    ...
    filterState.forEach((group, i) => {
      if (group.values.filter(value => value.isChecked)?.length) {
        activeGroupId = activeGroupId || i;
        activeGroupCount++;
      }
    });

    if (activeGroupCount === 1) {
      filterState[activeGroupId].values = activeteAllValuesInGroup(filterState[activeGroupId].values);
    } else {
      const activeFilterGroup = filterState.filter(group => group.key_en === this.activeFilterGroup)[0];

      if (activeFilterGroup) {
        activeFilterGroup.values = activeteAllValuesInGroup(activeFilterGroup.values)
        ;
      }
    }
  }
